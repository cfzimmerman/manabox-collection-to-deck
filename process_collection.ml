...
module Process_Collection: PROCESS_COLLECTION   =
  struct
    type csv_row = string array
    type csv = csv_row array 
    type output_format = 
    |   Printed | CSV of string
    ...
    let generate_output_csv (out_filename : string) (csv : csv) (target_fields : int list) : unit =
    ...
    let rec get_header_col_indices (header : string array) (target_columns : string list) : int list =
    ...
    let print_deck_from_collection (input_filename : string) (target_columns : string list) : unit =
